...
// setup_compassmot - sets compass's motor interference parameters
void Copter::user_compassmot()
{
#if FRAME_CONFIG == HELI_FRAME
    // compassmot not implemented for tradheli
    hal.console->printf("compassmot not implemented for tradheli\n");
    return;
#else
    int8_t   comp_type;                 // throttle or current based compensation
    Vector3f compass_base[COMPASS_MAX_INSTANCES];           // compass vector when throttle is zero
    ...
    //gcs_chan.send_text(MAV_SEVERITY_CRITICAL, "COMPASS/MOT CAL initiated\n");
    hal.console->printf("COMPASS/MOT CAL initiated\n");

    // exit immediately if we are already in compassmot
    if (ap.compass_mot) {
        // ignore restart messages
								hal.console->printf("already in compassmot\n");
								return;
    } else {
        ap.compass_mot = true;
    }
    ...
    // check compass is enabled
    if (!AP::compass().enabled()) {
								hal.console->printf("Compass disabled\n");
        ap.compass_mot = false;
        return;
    }
    ...
    for (uint8_t i=0; i<compass.get_count(); i++) {
        if (!compass.healthy(i)) {
								    hal.console->printf("Check compass\n");
            ap.compass_mot = false;
            return;
        }
    }
    ...
    // check if radio is calibrated
    if (!arming.rc_calibration_checks(true)) {
								hal.console->printf("RC not calibrated\n");
        ap.compass_mot = false;
        return;
    }
    ...
    while (command_ack_start == command_ack_counter && compass.healthy() && motors->armed()) {
        ...
        if (AP_HAL::millis() - last_send_time > 2000) {
            last_send_time = AP_HAL::millis();
    				    ...
    				    hal.console->printf("command_ack_start:%u,command_ack_counter:%u\n",command_ack_start,command_ack_counter);
			         rc().get_pwm((uint8_t)10,usr_rc10);
            if(usr_rc10 <= 1200)
    ...
    hal.console->printf("Stopping Motors\n");
    hal.console->printf("command_ack_start:%u,command_ack_counter:%u\n",command_ack_start,command_ack_counter);
    ...
    if (updated) {
        ...
        compass.save_motor_compensation();
        // display success message
				    hal.console->printf("Calibration successful\n");
        notify.set_led_override((float)0);
    } else {
        // compensation vector never updated, report failure
				    hal.console->printf("Calibration Failed\n");
        AP_Notify::handle_rgb((uint8_t)255, (uint8_t)0, (uint8_t)0);
        compass.motor_compensation_type(AP_COMPASS_MOT_COMP_DISABLED);
    }
